...
export function objectCode(obj, sw, sh, { resolveAsset }) {
  ...
  switch (obj.type) {
    ...
    case 'rectangle':
      {
        const rw = (obj.width / sw) * FRAME_WIDTH,
          rh = (obj.height / sh) * FRAME_HEIGHT;
      ...
      if (hasStroke)
        lines.push(
          `${n}.set_stroke(color=${stroke}, width=${sw2}${strokeOpacityArg(obj, opacity)})`
        );
      break;
    case 'square':
      if (obj.cornerRadius > 0) {
    ...
    case 'arc': {
      ...
      lines.push(
        `${n} = Arc(radius=${r.toFixed(3)}, start_angle=${+obj.startAngle || 0} * DEGREES, angle=${+obj.sweepAngle || 0} * DEGREES)`
      );
      lines.push(
        `${n}.set_stroke(color=${hex(obj.stroke) || hex(obj.fill) || '"#FFFFFF"'}, width=${sw2}${strokeOpacityArg(obj, opacity)})`
      );
      break;
    }
    case 'sector': {
      const r = (safeNum(obj.radius, 70) / sw) * FRAME_WIDTH;
      lines.push(
        `${n} = Sector(radius=${r.toFixed(3)}, start_angle=${+obj.startAngle || 0} * DEGREES, angle=${+obj.sweepAngle || 0} * DEGREES)`
      );
      if (hasFill)
        lines.push(`${n}.set_fill(color=${fill}, opacity=${fillOpacityExpr(obj, opacity)})`);
      if (hasStroke)
    ...
    case 'double_arrow': {
      ...
      lines.push(
        `${n} = DoubleArrow(start=LEFT * ${half}, end=RIGHT * ${half}, color=${hex(obj.fill) || '"#EF4444"'}, buff=0, stroke_width=${sw2})`
      );
      break;
    }
    case 'ellipse':
      lines.push(
        `${n} = Ellipse(width=${((obj.width / sw) * FRAME_WIDTH).toFixed(3)}, height=${((obj.height / sh) * FRAME_HEIGHT).toFixed(3)})`
      );
      if (hasFill)
        lines.push(`${n}.set_fill(color=${fill}, opacity=${fillOpacityExpr(obj, opacity)})`);
      if (hasStroke)
    ...
    case 'bezier': {
      ...
      lines.push(
        `${n}.set_stroke(color=${stroke || hex(obj.fill) || '"#F472B6"'}, width=${sw2}${strokeOpacityArg(obj, opacity)})`
      );
      break;
    }
    case 'parametric': {
    ...
    case 'matrix': {
      const data =
        Array.isArray(obj.matrixData) && obj.matrixData.length && Array.isArray(obj.matrixData[0])
          ? obj.matrixData
          : [
              ['1', '0'],
              ['0', '1'],
      ...
      const body = data
        .map((row) => `[${row.map((c) => `"${safeMatrixEntry(c)}"`).join(', ')}]`)
        .join(', ');
    ...
    case 'table': {
      const data =
        Array.isArray(obj.cellData) && obj.cellData.length && Array.isArray(obj.cellData[0])
          ? obj.cellData
          : [
              ['1', '2'],
              ['3', '4'],
      ...
      const labelArr = (arr) =>
        `[${arr.map((s) => `${wrap}("${safeMatrixEntry(s)}")`).join(', ')}]`;
      let args = `[${body}]`;
      if (Array.isArray(obj.rowLabels) && obj.rowLabels.length)
        args += `, row_labels=${labelArr(obj.rowLabels)}`;
      if (Array.isArray(obj.colLabels) && obj.colLabels.length)
        args += `, col_labels=${labelArr(obj.colLabels)}`;
    ...
    case 'line':
      lines.push(
        `${n} = Line(LEFT * ${((obj.width / 2 / sw) * FRAME_WIDTH).toFixed(3)}, RIGHT * ${((obj.width / 2 / sw) * FRAME_WIDTH).toFixed(3)})`
      );
      lines.push(
        `${n}.set_stroke(color=${hex(obj.stroke) || hex(obj.fill) || '"#FFFFFF"'}, width=${safeNum(obj.strokeWidth, 3)})`
      );
      break;
    case 'arrow': {
      ...
      lines.push(
        `${n} = Arrow(start=LEFT * ${halfLen}, end=RIGHT * ${halfLen}, color=${hex(obj.fill) || '"#EF4444"'}, buff=0, tip_length=${tipLen}, stroke_width=${sw2}, max_tip_length_to_length_ratio=0.15)`
      );
      break;
    }
    case 'counter': {
    ...
    case 'dot':
      lines.push(
        `${n} = Dot(radius=${((obj.width / 2 / sw) * FRAME_X_RADIUS).toFixed(3)}, color=${fill})`
      );
      break;
    case 'dot_grid': {
      const c = safeNum(obj.gridCols, 5),
        r = safeNum(obj.gridRows, 5),
        sp = (safeNum(obj.dotSpacing, 40) / sw) * FRAME_WIDTH;
      lines.push(
    ...
    case 'image':
      lines.push(
        `${n} = ImageMobject("${resolveAsset(obj, 'png')}").scale_to_fit_width(${((obj.width / sw) * FRAME_WIDTH).toFixed(3)})`
      );
      break;
    case 'svg_asset':
      lines.push(
        `${n} = SVGMobject("${resolveAsset(obj, 'svg')}").scale_to_fit_width(${((obj.width / sw) * FRAME_WIDTH).toFixed(3)})`
      );
      break;
    case 'latex': {
    ...
    case 'axes': {
      ...
      lines.push(
        `${n} = Axes(x_range=[${xr[0]}, ${xr[1]}, ${xr[2] ?? 1}], y_range=[${yr[0]}, ${yr[1]}, ${yr[2] ?? 1}], x_length=${((obj.width / sw) * FRAME_WIDTH).toFixed(1)}, y_length=${((obj.height / sh) * FRAME_HEIGHT).toFixed(1)}, tips=True)`
      );
      if (obj.graphs && obj.graphs.length > 0) {
        for (const g of obj.graphs) {
          ...
          if (g.riemann && g.riemann.enabled) {
            ...
            const rdx =
              Number.isFinite(g.riemann.dx) && g.riemann.dx > 0
                ? g.riemann.dx
                : (rxMax - rxMin) / 10;
          ...
          if (g.tangent && g.tangent.enabled) {
            ...
            const tlen =
              Number.isFinite(g.tangent.length) && g.tangent.length > 0 ? g.tangent.length : 2;
    ...
    case 'numberplane': {
      ...
      lines.push(
        `${n} = NumberPlane(x_range=[${xr[0]}, ${xr[1]}, ${xs}], y_range=[${yr[0]}, ${yr[1]}, ${ys}], x_length=${((obj.width / sw) * FRAME_WIDTH).toFixed(1)}, y_length=${((obj.height / sh) * FRAME_HEIGHT).toFixed(1)})`
      );
      break;
    }
    case 'complex_plane': {
      ...
      const yr = obj.yRange || [-2, 2, 1];
      lines.push(
        `${n} = ComplexPlane(x_range=[${xr[0]}, ${xr[1]}, ${xr[2] ?? 1}], y_range=[${yr[0]}, ${yr[1]}, ${yr[2] ?? 1}], x_length=${((obj.width / sw) * FRAME_WIDTH).toFixed(1)}, y_length=${((obj.height / sh) * FRAME_HEIGHT).toFixed(1)})`
      );
      break;
    }
    case 'polar_plane': {
      ...
      lines.push(
        `${n} = PolarPlane(radius_max=${rMax}, radius_step=${rStep}, azimuth_units=${az}, size=${((Math.min(obj.width, obj.height) / sw) * FRAME_WIDTH).toFixed(1)})`
      );
      break;
    }
    case 'numberline': {
      const xr = obj.xRange || [-5, 5, 1];
      lines.push(
        `${n} = NumberLine(x_range=[${xr[0]}, ${xr[1]}, ${xr[2] ?? 1}], length=${((obj.width / sw) * FRAME_WIDTH).toFixed(1)})`
      );
      break;
    }
    default:
