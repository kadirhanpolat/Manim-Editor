...
function addGraphVertexAuto() {
  if (!obj.value) return;
  store.addGraphVertex(obj.value.id);
}
function removeGraphVertex(v) {
  if (!obj.value) return;
  store.removeGraphVertex(obj.value.id, v);
}
function renameGraphVertex(oldV, newV) {
  if (!obj.value) return;
  const nv = graphVertexName(newV);
  if (nv && nv !== oldV) store.renameGraphVertex(obj.value.id, oldV, nv);
}
function addGraphEdgeFromUI() {
  if (!obj.value) return;
  const a = newEdgeFrom.value, b = newEdgeTo.value;
  if (a && b && a !== b) { store.addGraphEdge(obj.value.id, a, b); newEdgeFrom.value = ''; newEdgeTo.value = ''; }
}
function removeGraphEdge(a, b) {
  if (!obj.value) return;
  store.removeGraphEdge(obj.value.id, a, b);
}
</script>
