...
export function axesYLineCfg(obj, ctx) {
  const L = ctx.live(obj);
  const h = L ? L.h : obj.height * ctx.vs;
  return {
...
export function axesAreaRiemann(obj, ctx) {
  ...
  for (const graph of obj.graphs) {
    ...
    if (graph.area && graph.area.enabled) {
      ...
      if (pts.length >= 4) {
        ...
        areas.push({
          points: pts,
          closed: true,
          fill: graph.area.color || graph.color || '#f59e0b',
          opacity: graph.area.opacity ?? 0.5,
          listening: false,
        });
      }
    }
    if (graph.riemann && graph.riemann.enabled) {
      ...
      const dx =
        Number.isFinite(graph.riemann.dx) && graph.riemann.dx > 0
          ? graph.riemann.dx
          : (r1 - r0) / 10;
      ...
      for (let x = r0; x < r1 - 1e-9; x += dx) {
        ...
        rects.push({
          ...
          width: right - left,
          height: cy0 - toCy(y),
          fill: graph.riemann.color || graph.color || '#f59e0b',
          opacity: 0.45,
          stroke: '#fff',
          strokeWidth: 0.5,
          listening: false,
    ...
    if (graph.tangent && graph.tangent.enabled) {
      ...
      if (Number.isFinite(y0) && Number.isFinite(slope)) {
        ...
        tangents.push({
          ...
          stroke: graph.tangent.color || graph.color || '#f59e0b',
          strokeWidth: 2,
          listening: false,
        });
      }
