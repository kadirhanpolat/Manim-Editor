...
export function bgConfig(ctx) {
  return {
    x: ctx.ox,
    y: ctx.oy,
    width: ctx.stg.width * ctx.vs,
    height: ctx.stg.height * ctx.vs,
...
export function centerH(ctx) {
  ...
  return {
    points: [
      ctx.ox,
      ctx.oy + (ctx.stg.height * ctx.vs) / 2,
      ctx.ox + ctx.stg.width * ctx.vs,
      ctx.oy + (ctx.stg.height * ctx.vs) / 2,
    ],
...
export function centerV(ctx) {
  ...
  return {
    points: [
      ctx.ox + (ctx.stg.width * ctx.vs) / 2,
      ctx.oy,
      ctx.ox + (ctx.stg.width * ctx.vs) / 2,
      ctx.oy + ctx.stg.height * ctx.vs,
    ],
    stroke: ctx.themeAccent,
    strokeWidth: 0.5,
    opacity: gridOpacity + 0.06,
