...
describe('setClipAudio', () => {
  it('adds audio field to a clip', () => {
    const clip = store.addClip(0, { type: 'move', startTime: 0, duration: 2, sourceId: 'obj1', params: {} });
    store.setClipAudio(clip.id, { type: 'gtts', text: 'Merhaba', lang: 'tr', syncMode: 'auto', status: 'pending' });
  ...
  it('auto syncMode updates clip.duration when audio becomes ready', () => {
    ...
    store.setClipAudio(clip.id, { type: 'gtts', syncMode: 'auto', status: 'ready', duration: 3.5, src: '/data/assets/audio/x.wav' });
    const found = store.project.tracks[0].clips.find(c => c.id === clip.id);
  ...
  it('manual syncMode does not change clip.duration when audio becomes ready', () => {
    const clip = store.addClip(0, { type: 'move', startTime: 0, duration: 2, sourceId: 'obj1', params: {} });
    store.setClipAudio(clip.id, { type: 'file', syncMode: 'manual', status: 'ready', duration: 4.0, src: '/data/assets/audio/x.wav' });
    const found = store.project.tracks[0].clips.find(c => c.id === clip.id);
...
describe('hasPendingAudio getter', () => {
  ...
  it('returns false when all audio is ready', () => {
    const clip = store.addClip(0, { type: 'move', startTime: 0, duration: 2, sourceId: 'obj1', params: {} });
    store.setClipAudio(clip.id, { type: 'gtts', syncMode: 'auto', status: 'ready', src: '/data/assets/audio/x.wav' });
    expect(store.hasPendingAudio).toBe(false);
  });
});
