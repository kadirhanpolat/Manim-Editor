...
describe('client-side codegen (generateManimScript)', () => {
  it('generates self.play for a move clip', () => {
    ...
    const project = makeProject(
      [{ id: objId, type: 'circle', name: 'c1', x: 960, y: 540, width: 100, height: 100, fill: '#ff0000', stroke: 'transparent', strokeWidth: 2, opacity: 1, rotation: 0, enterTime: 0, duration: 3, enterAnim: 'none', exitAnim: 'none', zOrder: 0 }],
      [{ id: 'clip_1', type: 'move', sourceId: objId, startTime: 0.5, duration: 1, easing: 'ease_in_out', params: { targetX: 200, targetY: 300 }, parallel: false, lag_ratio: 0 }]
    );
  ...
  it('generates self.play for a scale clip with non-default easing', () => {
    ...
    const project = makeProject(
      [{ id: objId, type: 'rectangle', name: 'r1', x: 960, y: 540, width: 200, height: 100, fill: '#0000ff', stroke: 'transparent', strokeWidth: 2, opacity: 1, rotation: 0, enterTime: 0, duration: 3, enterAnim: 'none', exitAnim: 'none', zOrder: 0 }],
      [{ id: 'clip_2', type: 'scale', sourceId: objId, startTime: 0, duration: 1.5, easing: 'ease_in_cubic', params: { targetScaleX: 2, targetScaleY: 2 }, parallel: false, lag_ratio: 0 }]
    );
    const code = generateManimScript(project);
  ...
  it('generates FadeOut for a fade clip targeting opacity 0', () => {
    ...
    const project = makeProject(
      [{ id: objId, type: 'square', name: 's1', x: 960, y: 540, width: 150, height: 150, fill: '#ffffff', stroke: 'transparent', strokeWidth: 2, opacity: 1, rotation: 0, enterTime: 0, duration: 3, enterAnim: 'none', exitAnim: 'none', zOrder: 0 }],
      [{ id: 'clip_3', type: 'fade', sourceId: objId, startTime: 1, duration: 0.5, easing: 'ease_in_out', params: { targetOpacity: 0 }, parallel: false, lag_ratio: 0 }]
    );
    const code = generateManimScript(project);
