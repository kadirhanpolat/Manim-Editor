...
describe('arc codegen', () => {
  const script = (o) => generateManimScript(makeProject([o]));
  const roundTrip = (o) =>
    parseManimScript(generateManimScript(makeProject([o])), SW, SH).objects[0];
  it('emits Arc with radius + angles in DEGREES', () => {
    ...
    expect(s).toMatch(
      /= Arc\(radius=[\d.]+, start_angle=[-\d.]+ \* DEGREES, angle=[-\d.]+ \* DEGREES\)/
    );
  });
...
describe('sector codegen', () => {
  ...
  it('emits Sector with radius + angles in DEGREES', () => {
    ...
    expect(s).toMatch(
      /= Sector\(radius=[\d.]+, start_angle=[-\d.]+ \* DEGREES, angle=[-\d.]+ \* DEGREES\)/
    );
