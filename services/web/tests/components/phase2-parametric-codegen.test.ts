...
describe('parametric codegen', () => {
  it('emits single-line ParametricFunction with t_range', () => {
    ...
    expect(s).toMatch(
      /= ParametricFunction\(lambda t: np\.array\(\[np\.cos\(t\), np\.sin\(t\), 0\]\), t_range=\[0, 6\.283\], color="#10b981", stroke_width=4\)/
    );
  });
  ...
  it('round-trips a comma-containing (multi-arg) expression', () => {
    const o = parseManimScript(
      generateManimScript(makeProject([makeObj({ xExpr: 'np.power(t, 2)', yExpr: 'np.sin(t)' })])),
      SW,
      SH
    ).objects[0];
  ...
  it('a parametric and a heart in the same scene each round-trip to their own type', () => {
    ...
    const objs = parseManimScript(
      generateManimScript(makeProject([makeObj(), heart])),
      SW,
      SH
    ).objects;
